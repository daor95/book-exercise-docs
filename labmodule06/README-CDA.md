...
The implementation developed in this lab module enables MQTT-based communication between the Constrained Device Application (CDA) and a MQTT broker by using the MqttClientConnector class. This connector handles all publish-subscribe functionality required for the CDA to communicate effectively within an IoT environment. It connects to the broker using credentials and configuration values defined in a property file, manages topic subscriptions, publishes messages with configurable QoS levels, and properly handles connection lifecycle events such as connecting, disconnecting, and pinging to maintain session state. It also registers internal callbacks for message arrival, publish confirmation, subscription acknowledgment, and connection events. The connector was then integrated into the DeviceDataManager to allow automated MQTT initialization and shutdown based on the lifecycle of the device application. This ensures that topics are subscribed to at start-up and cleanly unsubscribed and disconnected at shutdown, enabling reliable and consistent communication.

How does your implementation work?
The implementation works by establishing a full MQTT communication flow, starting with configuration loading, instantiating the MQTT client with a unique client ID, and registering all required event callbacks. Methods such as publishMessage(), subscribeToTopic(), and unsubscribeFromTopic() are designed to validate inputs and ensure QoS compliance before interacting with the broker. Tests were written and executed to confirm the correct handling of all MQTT 3.1.1 control packets, including those for QoS 1 and 2, using a custom integration test named MqttClientControlPacketTest. This test demonstrated the generation of all 14 control packet types, including PUBLISH/PUBACK for QoS 1 and the full PUBREC → PUBREL → PUBCOMP handshake required for QoS 2. Additionally, Wireshark was used to verify the transmission of each control packet at the protocol level.
